    ...
    <? if ($news_item['show'] == 1 || $this->session->userdata('logged_in')) {
      ...
      <div style="display:none">
        <form class="rename_news_form" id="rename_news_form_<?= $news_item['id'] ?>" method="post" action="">
          ...
          <p>
            Title<br/>
            <input type="text" id="rename_title_field_<?= $news_item['id'] ?>" value="<?= $news_item['title'] ?>"
                   style="width:250px"/>
          </p>
          ...
          <p>
            Text<br/>
            <textarea name="text"
                      id="rename_text_field_<?= $news_item['id'] ?>"><?= $news_item['text_raw'] ?></textarea><br/>
          </p>
          <p>
      ...
      <? if ($this->session->userdata('logged_in')) { ?>
        ...
        <? if ($news_item['show'] == 1) { ?>
          <p>This news is visible.
            <a href="<?= site_url('/news/hide/') ?>/<? echo $news_item['id'] . '/#' . ($prev_id - 1); ?>">Click here to
              hide this news</a></p><br/>
        <? } else { ?>
          <p>This news is hidden.
            <a href="<?= site_url('/news/show/') ?>/<? echo $news_item['id'] . '/#' . ($prev_id - 1); ?>">Click here to
              make this visible</a></p><br/>
        <? } ?>
      <? } ?>
      <? $prev_id += 1;
      if (!($key + 1 == count($news))) {
