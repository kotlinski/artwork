    ...
    <table class="projects-table">
      ...
      <?php foreach ($projects as $index => $project): ?>
        <tr>
          ...
          <td>
            <a href="#" class="delete-link" data-id="<?= $project['id'] ?>" onclick="return confirm('Are you sure you want to delete this project?')">delete</a>
          </td>
        </tr>
      <?php endforeach; ?>
      </tbody>
  ...
  <?php foreach ($projects as $project): ?>
    <form method="post" action="<?= base_url('artwork/update/' . $project['id']) ?>" class="project-card"
      ...
      <div style="display: flex; gap: 1em; align-items: center; margin-top: 8px;">
        <label>From Year: <input type="number" name="start_year" value="<?= esc((isset($project['start_year']) && $project['start_year'] != 0) ? $project['start_year'] : '') ?>"
                                 min="1900"
                                 max="2100" style="width: 80px;"></label>
        <label>To Year: <input type="number" name="end_year" value="<?= esc((isset($project['end_year']) && $project['end_year'] != 0) ? $project['end_year'] : '') ?>" min="1900"
                               max="2100" style="width: 80px;"></label>
      </div>
      <div style="display: flex; gap: 1em; align-items: center; margin-top: 8px;">
        <label>Image Left:
          <select name="image_left" style="width: 122px;">
            ...
            <?php if (isset($project['images']) && is_array($project['images'])): ?>
              <?php foreach ($project['images'] as $image): ?>
                <option
                  value="<?= esc($image['id'] ?? '') ?>" <?= (isset($project['image_left']) && $project['image_left'] == ($image['id'] ?? null)) ? 'selected' : '' ?>>
                  <?= esc($image['file_id'] ?? '') ?>
                </option>
              <?php endforeach; ?>
            <?php endif; ?>
        ...
        <label>Image Mid:
          <select name="image_mid" style="width: 122px;">
            ...
            <?php if (isset($project['images']) && is_array($project['images'])): ?>
              <?php foreach ($project['images'] as $image): ?>
                <option
                  value="<?= esc($image['id'] ?? '') ?>" <?= (isset($project['image_mid']) && $project['image_mid'] == ($image['id'] ?? null)) ? 'selected' : '' ?>>
                  <?= esc($image['file_id'] ?? '') ?>
                </option>
              <?php endforeach; ?>
            <?php endif; ?>
          ...
          <select name="image_right" style="width: 122px;">
            ...
            <?php if (isset($project['images']) && is_array($project['images'])): ?>
              <?php foreach ($project['images'] as $image): ?>
                <option
                  value="<?= esc($image['id'] ?? '') ?>"
                  <?= (isset($project['image_right']) && $project['image_right'] == ($image['id'] ?? null)) ? 'selected' : '' ?>>
                  <?= esc($image['file_id'] ?? '') ?>
                </option>
              <?php endforeach; ?>
            <?php endif; ?>
