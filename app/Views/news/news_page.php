...
<div class='contained'>
  <?php foreach ($news_items as $idx => $item): ?>
    <article id="news-<?= $item['slug'] ?>" class="news-item" data-project-id="<?= esc($item['project_id'] ?? '') ?>"
      ...
      <?php if (!empty($item['main_image'])): ?>
        <?php
        $mainImageThumb = $item['main_image_thumb'] ?? ($item['main_image_medium'] ?? $item['main_image']);
        $mainImageThumb2x = $item['main_image_thumb2x'] ?? ($item['main_image_large'] ?? $mainImageThumb);
        $mainImageFull = $item['main_image'] ?? $mainImageThumb;
        // Prefer controller-provided thumb dims; otherwise derive from stored original dims.
        $thumbW = isset($item['main_image_width']) && (int)$item['main_image_width'] > 0 ? (int)$item['main_image_width'] : null;
        $thumbH = isset($item['main_image_height']) && (int)$item['main_image_height'] > 0 ? (int)$item['main_image_height'] : null;
        if (!$thumbW || !$thumbH) {
          $origW = isset($item['width_px']) ? (int)$item['width_px'] : 0;
          $origH = isset($item['height_px']) ? (int)$item['height_px'] : 0;
          if ($origW > 0 && $origH > 0) {
            $scale = min(122 / $origW, 122 / $origH, 1.0);
            $thumbW = (int)round($origW * $scale);
            $thumbH = (int)round($origH * $scale);
          } else {
            // Last-resort square fallback so space is always reserved.
            $thumbW = 122;
            $thumbH = 122;
          }
        }
